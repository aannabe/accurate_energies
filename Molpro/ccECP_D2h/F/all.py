...
for i in folders:
	for j in basis:
		...
		try:
			...
			if i==best_method and i=='ccsdt-q':
				#scf[j].loc[i]=np.nan
				#corr[j].loc[i]=np.nan
				print('Estimating CCSDT(Q) at basis:', j)
				scf[j].loc[i]=scf[j].loc['uccsd-t']
				eta=corr[j-1].loc['ccsdt-q']/corr[j-1].loc['uccsd-t']
				#print("Eta: ", eta)
				corr[j].loc[i]=eta*(corr[j].loc['uccsd-t'])
			elif i==best_method and i=='fci':
				#scf[j].loc[i]=np.nan
				#corr[j].loc[i]=np.nan
				print('Estimating FCI at basis:', j)
				scf[j].loc[i]=scf[j].loc['uccsd-t']
				eta=corr[j-1].loc['fci']/corr[j-1].loc['ccsdt-q']
				#print("Eta: ", eta)
				corr[j].loc[i]=eta*(corr[j].loc['ccsdt-q'])
			else:
				scf[j].loc[i]=np.nan
				corr[j].loc[i]=np.nan
...
scf['CBS']=frmtr.format("{0:.1u}", my_scf)
